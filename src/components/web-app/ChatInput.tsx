...
export const ChatInput = React.forwardRef<HTMLDivElement, ChatInputProps>(
  (
    ...
    useEffect(() => {
      if (expanded && !isRecording && !disabled) {
      ...
      <>
        ...
        >
          ...
          <div
            ...
            className={cn(
              "relative w-full border border-border/80 bg-background/50 backdrop-blur-xl shadow-sm focus-within:border-primary/40 focus-within:ring-1 focus-within:ring-primary/20 hover:border-border transition-all duration-300 z-10",
          ...
          >
            ...
            <textarea
              ...
              className={cn(
                "prompt-scrollbar absolute top-0 inset-x-0 z-[1] w-full resize-none bg-transparent pl-5 pr-14 py-4 text-[15px] leading-relaxed text-foreground outline-none placeholder:font-medium placeholder:text-muted-foreground/70 cursor-text",
            ...
            <button
              ...
              className={cn(
                "absolute inset-x-0 top-0 z-[1] cursor-text pl-5 pr-14 py-[17px] text-left text-[15px] font-medium text-muted-foreground/70 outline-none",
            ...
            <div
              className={cn(
                "absolute bottom-2.5 left-3 right-14 z-[10] flex items-center gap-1 transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            >
              <div className="relative">
                <button
                  ...
                  className={cn(
                    "group flex items-center gap-1.5 rounded-full px-2.5 py-1 text-foreground/60 transition-all duration-200 outline-none hover:bg-accent hover:text-foreground cursor-default",
                    isModelSelectOpen ? "bg-accent text-foreground" : ""
                  )}
                  aria-label={`Select model. Current: ${selectedModel}`}
                >
                  <ModelIcon model={selectedModel} className="size-4 opacity-70 group-hover:opacity-100 transition-opacity" />
                ...
                <div
                  ...
                  className={cn(
                    "absolute bottom-full left-0 mb-3 z-50 w-48 rounded-2xl border border-border bg-card/95 p-1.5 shadow-xl backdrop-blur-md flex flex-col gap-0.5 transition-all duration-400 cursor-default",
                    isModelSelectOpen
                ...
                >
                  <div className="relative flex flex-col gap-1">
                    <div style={hoverStyle} className="absolute left-0 right-0 top-0 h-9 -z-10 rounded-xl bg-accent pointer-events-none" />
                    {models.map((model, idx) => (
                      <button
                        ...
                        onMouseEnter={() => {
                          setHoverStyle((prev) => ({
                            opacity: 1, transform: `translateY(${idx * 40}px) scale(1)`,
                        ...
                        className="group relative flex h-9 w-full items-center justify-between rounded-xl px-3 py-2 text-left text-sm font-medium text-foreground/80 outline-none active:scale-[0.98] cursor-default"
                      >
                        <span className="flex items-center gap-2.5">
                          <ModelIcon model={model} className="size-4 opacity-85 group-hover:opacity-100 transition-opacity" />
                          {model}
                        </span>
                      </button>
                    ))}
              ...
              <button
                ...
                className="group flex items-center gap-1.5 rounded-full px-2.5 py-1 text-foreground/60 transition-all duration-200 hover:bg-accent hover:text-foreground outline-none cursor-default"
              >
              ...
              <button
                ...
                className="ml-auto flex size-8 items-center justify-center rounded-full text-foreground/60 transition-all duration-200 hover:bg-accent hover:text-foreground outline-none cursor-default disabled:opacity-40 disabled:pointer-events-none"
              >
                <PlusIcon />
              </button>
            </div>
            ...
            <div
              className={cn(
                "absolute right-14 bottom-3 z-[10] flex h-8 items-center justify-end gap-[3px] transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
            ...
            <button
              ...
              className={cn(
                "absolute right-2.5 bottom-2.5 z-[10] flex h-9 w-9 items-center justify-center rounded-full transition-all duration-300 outline-none focus-visible:ring-2 focus-visible:ring-ring cursor-default",
                (showArrow || showStop) ? "bg-primary text-primary-foreground shadow-sm hover:opacity-90" : "bg-muted text-foreground/60 hover:bg-accent hover:text-foreground",
                loading && "opacity-50 cursor-not-allowed"
              )}
            >
              <span className="relative flex h-full w-full items-center justify-center">
                {loading ? (
                   <Loader2 className="absolute h-4 w-4 animate-spin text-primary-foreground" />
                ) : (
                  <>
